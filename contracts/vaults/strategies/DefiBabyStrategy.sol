...
contract DefiBabyStrategy is StratManager, FeeManager, GasThrottler, ReentrancyGuard {
    ...
    constructor(
        ...
        } else if (address(lpToken0) == _usdt) {
            outputToLp0Route = [_outputToken, _usdt];
            wbnbToLp0Route = [_wbnb, _usdt];
        } else if (address(lpToken0) != _outputToken) {
            outputToLp0Route = [_outputToken, _usdt, address(lpToken0)];
            wbnbToLp0Route = [_wbnb, _usdt, address(lpToken0)];
        } else if (address(lpToken0) == _outputToken) {
        ...
        } else if (address(lpToken1) == _usdt) {
            outputToLp1Route = [_outputToken, _usdt];
            wbnbToLp1Route = [_wbnb, _usdt];
        } else if (address(lpToken1) != _outputToken) {
            outputToLp1Route = [_outputToken, _usdt, address(lpToken1)];
            wbnbToLp1Route = [_wbnb, _usdt, address(lpToken1)];
        } else if (address(lpToken1) == _outputToken) {
