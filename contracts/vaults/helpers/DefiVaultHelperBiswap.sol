...
contract DefiVaultHelperBiswap is ReentrancyGuard, IVaultHelper {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IDefiVault;

    IBiswapRouter02 public immutable router;
    ...
    function withdrawToTokens(address vault, uint256 withdrawAmount) public override nonReentrant {
        (IDefiVault _vault, IBiswapPair _pair) = _getVaultPair(vault);
    ...
    function depositWETHFromPrize(
        ...
        (, IBiswapPair _pair) = _getVaultPair(vault);
        ...
        if (_pair.token0() == address(WETH) || _pair.token1() == address(WETH)) {
            ...
            IBiswapRouter02(router).swapExactTokensForTokens(wethAmount, 0, _swapRoute, address(this), block.timestamp);
    ...
    function _getVaultPair(address vault) private view returns (IDefiVault _vault, IBiswapPair _pair) {
        _vault = IDefiVault(vault);

        IERC20 pairAddress = _vault.want();
        _pair = IBiswapPair(address(pairAddress));
    ...
    function _swapAndStake(
        ...
        uint32 swapFee = _pair.swapFee();
        uint256 swapAmountIn;
        if (isInputA) {
            swapAmountIn = _getSwapAmount(fullInvestment, reserveA, reserveB, swapFee);
        } else {
            swapAmountIn = _getSwapAmount(fullInvestment, reserveB, reserveA, swapFee);
        }
    ...
    {
        (, IBiswapPair pair) = _getVaultPair(vault);
        ...
        uint32 _swapFee = pair.swapFee();
        swapAmountIn = _getSwapAmount(fullInvestmentIn, reserveA, reserveB, _swapFee);
        swapAmountOut = router.getAmountOut(swapAmountIn, reserveA, reserveB, _swapFee);
