...
contract DefiVaultHelperBaby is ReentrancyGuard, IVaultHelper {
    ...
    function depositWETHFromPrize(
        ...
        if (_pair.token0() == address(WETH) || _pair.token1() == address(WETH)) {
            ...
            if (_pair.token0() == address(USDT) || _pair.token1() == address(USDT)) {
                _token = address(USDT);
                _swapRoute = wbnbToUsdtRoute;
            }
            require(_token != address(0), "VaultHelper: Token wasn't USDT");
